...
```{r}
load(here('results', 'compare_results.Rdata'))
```
...
plot_performances(df_1d, method = 'MM1D', metric = 'auROC')
```
...
improvement_plot %>%
  ggsave(filename = here('figs', 'improvements_train.pdf'), height = 4.5, width = 4)
```

But information is currently computed on the training set only. What about testing?

```{r}
info_gain <- read_info_test('information_gain_test.csv', '2-way information gain')
mutual_info_1d <- read_info_test('mutual_infomation_1d_test.csv', '1-way mutual information')
